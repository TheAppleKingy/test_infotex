...
type WalletQuery struct {
	...
	withSentTransactions     *TransactionQuery
	withRecievedTransactions *TransactionQuery
...
// QuerySentTransactions chains the current query on the "sent_transactions" edge.
func (wq *WalletQuery) QuerySentTransactions() *TransactionQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, wallet.SentTransactionsTable, wallet.SentTransactionsColumn),
		)
...
// QueryRecievedTransactions chains the current query on the "recieved_transactions" edge.
func (wq *WalletQuery) QueryRecievedTransactions() *TransactionQuery {
	query := (&TransactionClient{config: wq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := wq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(wallet.Table, wallet.FieldID, selector),
			sqlgraph.To(transaction.Table, transaction.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, wallet.RecievedTransactionsTable, wallet.RecievedTransactionsColumn),
		)
		fromU = sqlgraph.SetNeighbors(wq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (wq *WalletQuery) Clone() *WalletQuery {
	...
	return &WalletQuery{
		...
		withSentTransactions:     wq.withSentTransactions.Clone(),
		withRecievedTransactions: wq.withRecievedTransactions.Clone(),
...
// WithSentTransactions tells the query-builder to eager-load the nodes that are connected to
// the "sent_transactions" edge. The optional arguments are used to configure the query builder of the edge.
func (wq *WalletQuery) WithSentTransactions(opts ...func(*TransactionQuery)) *WalletQuery {
	query := (&TransactionClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wq.withSentTransactions = query
	return wq
}

// WithRecievedTransactions tells the query-builder to eager-load the nodes that are connected to
// the "recieved_transactions" edge. The optional arguments are used to configure the query builder of the edge.
func (wq *WalletQuery) WithRecievedTransactions(opts ...func(*TransactionQuery)) *WalletQuery {
	query := (&TransactionClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wq.withRecievedTransactions = query
	return wq
}
...
func (wq *WalletQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Wallet, error) {
	var (
		...
		_spec       = wq.querySpec()
		loadedTypes = [2]bool{
			wq.withSentTransactions != nil,
			wq.withRecievedTransactions != nil,
		}
	)
	...
	if query := wq.withSentTransactions; query != nil {
		if err := wq.loadSentTransactions(ctx, query, nodes,
			func(n *Wallet) { n.Edges.SentTransactions = []*Transaction{} },
			func(n *Wallet, e *Transaction) { n.Edges.SentTransactions = append(n.Edges.SentTransactions, e) }); err != nil {
			return nil, err
		}
	}
	if query := wq.withRecievedTransactions; query != nil {
		if err := wq.loadRecievedTransactions(ctx, query, nodes,
			func(n *Wallet) { n.Edges.RecievedTransactions = []*Transaction{} },
			func(n *Wallet, e *Transaction) {
				n.Edges.RecievedTransactions = append(n.Edges.RecievedTransactions, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (wq *WalletQuery) loadSentTransactions(ctx context.Context, query *TransactionQuery, nodes []*Wallet, init func(*Wallet), assign func(*Wallet, *Transaction)) error {
...
func (wq *WalletQuery) loadRecievedTransactions(ctx context.Context, query *TransactionQuery, nodes []*Wallet, init func(*Wallet), assign func(*Wallet, *Transaction)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Wallet)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
